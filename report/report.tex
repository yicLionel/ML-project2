...
result was obtained using HOG, additional features, and EfficientNet-V2-L embeddings, 
...
HOG + additional + EffNet-V2-L & 0.973 & 0.973 \\
\hline
\end{tabular}
...
on the same HOG, additional features, and EfficientNet-V2-L feature set. 
...
\hline
\end{tabular}
\caption{Selected Task 1 held-out validation comparison of models using HOG, additional features, and EfficientNet-V2-L embeddings.}
...
descriptors with ImageNet-pretrained CNN embeddings, especially HOG, additional
features, and EfficientNet-V2-L. This suggests that low-level descriptors and
CNN embeddings captured complementary information. The coarse-to-fine progression
also shows that Task 2 required a stronger representation: broad shape and colour
cues were often enough for Task 1, but Task 2 depended more on subtle local
markings and texture patterns. Once EfficientNet-V2-L embeddings were included,
several classifiers achieved similar validation performance, which indicates that
the main gain came from improving the representation rather than only choosing a
...
In particular, HOG, additional features, and EfficientNet-V2-L embeddings gave the best overall performance.
