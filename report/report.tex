...
I experimented with \textbf{ResNet-50} and \textbf{EfficientNet} variants, with EfficientNet-V2-L giving the strongest representation in later experiments. 
...
Colour + HOG + additional + ResNet-50 & 0.897 & 0.897 \\
...
Initial experiments using only the provided low-level descriptors gave limited
performance, with the best provided-feature model reaching 40.5\% accuracy.
Adding CNN embeddings produced a much larger improvement, as shown in
Table~\ref{tab:task2-feature-results}. The strongest representation combined
HOG, additional features, and EfficientNet-V2-L embeddings, and this feature set
gave consistently strong performance across different classifiers.
...
\begin{tabular}{lcc}
\hline
Feature set (best model) & Acc. & Macro-F1 \\
\hline
Colour + HOG + additional, Random Forest & 0.405 & 0.392 \\
HOG + additional + ResNet-50, Extra Trees & 0.845 & 0.838 \\
HOG + additional + EffNet-V2-L, Random Forest & 0.976 & 0.975 \\
\hline
\end{tabular}
}
\caption{Task 2 held-out validation comparison between provided low-level features and the final deep-feature representation.}
\label{tab:task2-feature-results}
\end{table}
...
The improvement from ResNet-50 and EfficientNet-V2-M to EfficientNet-V2-L
suggests that representation capacity also mattered. The stronger model likely
captured richer visual patterns that transferred better to fine-grained bird
recognition. At the same time, the CNNs were used only as fixed ImageNet-pretrained feature extractors,
...
I declare that I used generative AI tools, including ChatGPT and Codex, 
to assist with project organisation, code structure, and report paraphrasing where appropriate. 
All experiments, results, analysis, and final report content were reviewed, verified, 
and submitted by me. No project materials or outputs were shared with any other student, staff member, or third party.
